...
import { AlertTriangle, CheckCircle2, Circle, RefreshCw, Ship, Users } from 'lucide-react';
...
export default function Activities() {
  ...
  async function setActivity(ids: string[], code: string) {
              ...
              {groupByBooking(boat.rows).map((group) => (
                <div key={group.bookingRef} className="rounded-xl border border-line bg-shell/40 p-2.5">
                  ...
                  <ul className="grid gap-1.5">
                    {group.rows.map((row) => (
                      <li key={row.passenger_id} className="flex flex-wrap items-center gap-2 rounded-lg border border-line bg-white px-2.5 py-2">
                        <span className="min-w-[8rem] flex-1 truncate text-sm font-black">{row.full_name}</span>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {types.map((type) => (
                            <button
                              ...
                              onClick={() => setActivity([row.passenger_id], type.code)}
                        ...
                        {canMark ? (
                          <div className="ml-auto flex gap-1">
                            <button
                              ...
                              className={`rounded-lg border px-2.5 py-1.5 text-xs font-black ${
                                row.activity_status === 'joined' ? 'border-accent bg-teal-50 text-accent' : 'border-line bg-white'
                              } disabled:opacity-50`}
                            >
                              Joined
                            </button>
